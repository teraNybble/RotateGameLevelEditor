...
bool TextureManager::loadPNGFromFile(const char* name, Rect& rect, bool& outHasAlpha, GLubyte** outData)
{
	png_structp png_ptr;
	png_infop info_ptr;
...
GLuint TextureManager::loadPNG(const char* path,int filter)
{
	...
	if (loadPNGFromFile(path, imageRect, hasAlpha, &image))
	{
		...
		glTexImage2D(GL_TEXTURE_2D, 0, (hasAlpha ? GL_RGBA : GL_RGB), imageRect.getWidth(), imageRect.getHeight(), 0, /*img.getFormat()*/(hasAlpha ? GL_RGBA : GL_RGB), GL_UNSIGNED_BYTE, image);
